...
const form = useForm({
  ...
  description_prefix: props.data.description_prefix,
});

const submit = () => {
  form.put(
    route('grade-descriptors.update', { grade_descriptor: props.data.id }),
  );
};
</script>

<template>
  <Head :title="`Ubah Predikat - ${data.name}`" />
