...
const submit = () => {
  form.put(
    route('indicators.subindicators.update', {
      indicator: props.indicator.id,
      subindicator: props.data.id,
    }),
  );
};
</script>
...
<template>
  <Head
    :title="`Indikator ${indicator.name} | Ubah Subindikator - ${data.name}`"
  />

  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-6">
        <Link :href="route('indicators.show', { indicator: indicator.id })">
          <svg
        ...
        >
          Indikator {{ indicator.name }} | Ubah Subindikator {{ data.name }}
        </h2>
      </div>
    </template>
