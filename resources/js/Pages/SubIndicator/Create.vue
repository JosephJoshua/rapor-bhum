...
const submit = () => {
  form.post(
    route('indicators.subindicators.store', { indicator: props.indicator.id }),
  );
};
</script>

<template>
  <Head :title="`Indikator ${indicator.name} | Tambah Subindikator`" />
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-6">
        ...
        >
          Indikator {{ indicator.name }} | Tambah Subindikator
        </h2>
      </div>
    </template>
